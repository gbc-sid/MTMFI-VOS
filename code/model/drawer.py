    ...
    def run_dataset(self, dataset, out_path, speedrun=False, restart=None):
        ...
        for sequence in dataset:
            if restart is not None and not restarted:
                ...
                restarted = True
            # 测试可视化用
            if sequence.name != "guitar-violin":
                continue
    ...
    def track(self, image):
        ...
        for (obj_id_l3, target_l3), (obj_id_l4, target_l4)\
            ...
            if target_l4.start_frame < self.current_frame:
                ...
                # Target model可视化
                img_l3 = np.array(s_l3.squeeze(0).permute(1, 2, 0).cpu())
                img_l4 = np.array(s_l4.squeeze(0).permute(1, 2, 0).cpu())
                img_l3 = img_l3/np.max(img_l3)*255
                img_l4 = img_l4 / np.max(img_l4) * 255
                zero_l3 = np.zeros(np.shape(img_l3))
                zero_l4 = np.zeros(np.shape(img_l4))
                if obj_id_l3 == 1:
                    img_l3_r = np.concatenate((zero_l3, zero_l3, img_l3), axis=2)
                    img_l4_r = np.concatenate((zero_l4, zero_l4, img_l4), axis=2)
                    img_l3, img_l4 = img_l3_r, img_l4_r
                if obj_id_l3 == 2:
                    img_l3_g = np.concatenate((zero_l3, img_l3, zero_l3), axis=2)
                    img_l4_g = np.concatenate((zero_l4, img_l4, zero_l4), axis=2)
                    img_l3 = img_l3_r + img_l3_g
                    img_l4 = img_l4_r + img_l4_g
                if obj_id_l3 == 3:
                    img_l3_b = np.concatenate((zero_l3, img_l3, img_l3), axis=2)
                    img_l4_b = np.concatenate((zero_l4, img_l4, img_l4), axis=2)
                    img_l3 = img_l3_r + img_l3_g + img_l3_b
                    img_l4 = img_l4_r + img_l4_g + img_l4_b
                if obj_id_l3 == 4:
                    img_l3_y = np.concatenate((img_l3, zero_l3, zero_l3), axis=2)
                    img_l4_y = np.concatenate((img_l4, zero_l4, zero_l4), axis=2)
                    img_l3 = img_l3_r + img_l3_g + img_l3_b + img_l3_y
                    img_l4 = img_l4_r + img_l4_g + img_l4_b + img_l4_y
                if obj_id_l3 == 5:
                    img_l3_p = np.concatenate((img_l3, zero_l3, img_l3), axis=2)
                    img_l4_p = np.concatenate((img_l4, zero_l4, img_l4), axis=2)
                    img_l3 = img_l3_r + img_l3_g + img_l3_b + img_l3_y + img_l3_p
                    img_l4 = img_l4_r + img_l4_g + img_l4_b + img_l4_y + img_l4_p
