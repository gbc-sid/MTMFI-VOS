...
from .optimizer import MinimizationProblem, GaussNewtonCG, GradientDescent
...
class DiscriminatorLoss(MinimizationProblem):
    ...
    def initialize(self):
        ...
        # vk对准确率的影响很大吗？
        # self.w = self.sample_weights[a].sqrt().view(-1, 1, 1, 1)

    def __call__(self, parameters: TensorList):

        s = self.net(self.x)
        s = F.interpolate(s, self.y_size, mode='bilinear', align_corners=False)
        # L2 LOSS改成SMOOTH L1(测试序列出现问题)
        # smooth = torch.where(abs(s - self.y)<1, abs(s - self.y), abs(s - self.y) ** 0.5)
        # residuals = self.w * abs(s - self.y) ** smooth
...
class Discriminator(nn.Module):
    ...
    def init(self, x, y):
        ...
        # resnet18+dv2017: feature(256, 30, 54), label(1, 480, 854)
        ...
        # code
        # optimizer = GradientDescent(problem, parameters, step_length=1e-3)
        problem.net.train()
        optimizer.run(self.init_iters) # self.init_iters
        problem.net.eval()

        x = self.project(x)  # Re-project samples with the new projection matrix

        # Initialize the memory
        # resnet18+dv2017: feature(c_channel, 30, 54), label(1, 480, 854)
        ...
        # optimizer = GradientDescent(problem, parameters, step_length=1e-3)
        problem.net.train()
        optimizer.run(self.update_iters) # self.update_iters
        problem.net.eval()
