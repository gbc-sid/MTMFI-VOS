...
class ConvBNReLU(nn.Module):
    def __init__(self, ic, oc, ks=3):
        super(ConvBNReLU, self).__init__()
        self.convbl = nn.Sequential(conv(ic, oc, ks), nn.BatchNorm2d(oc), relu())
...
class ARM(nn.Module):
    def __init__(self, ic, oc):
        ...
        self.convbrl = ConvBNReLU(ic, oc, 3)
        self.conv_atten = conv(oc, oc, 1, bias=False)
...
class TFFM(nn.Module):
    def __init__(self, fc, ic, oc):
        ...
        self.att = nn.Sequential(conv(nc, nc, 1), relu(), conv(nc, nc, 1, bias=False))
        self.transform = nn.Sequential(conv(nc, nc, 3), relu(), conv(nc, nc, 3), relu(), conv(nc, oc, 3), relu())

    def forward(self, ft, score1, score2, x=None):
        # arm1 = self.reduce(score1)
        # arm2 = self.reduce(score2)
        h = self.reduce(ft)
        # adaptive:只需给定输出输出，不需要设定kernel, stride
        hpool = F.adaptive_avg_pool2d(h, (1, 1)) if x is None else x
        h = adaptive_cat((h, score1, score2), dim=1, ref_tensor=0)
        h_atten = F.avg_pool2d(h, h.shape[-2:])
        h_atten = self.att(h_atten)
        h_feat = torch.mul(h, torch.sigmoid(h_atten))
        h = h + h_feat
        h = self.transform(h)
        return h, hpool
...
class SegNetwork(nn.Module):
    ...
    def forward(self, scores_l3, scores_l4, features, image_size):
        ...
        for i, L in enumerate(self.ft_channels):
            ...
            if multi_targets:
                h, hpool = self.TFFM[L](ft.repeat(num_targets, 1, 1, 1), s_l3, s_l4, x)
            else:
                h, hpool = self.TFFM[L](ft, s_l3, s_l4, x)
            # print('L:', L)
            # print('ft:', ft.size())
            # print('TSE:', h.size(), hpool.size())
            h = self.RRB1[L](h)
...
class SegNetwork_mini(nn.Module):
    def __init__(self, in_channels=1, out_channels=32, ft_channels=None, use_bn=False):
...
if __name__ == "__main__":
    ft = {'layer5': 2048, 'layer4': 1024, 'layer3': 512, 'layer2': 256}
    net = SegNetwork(1, 128, ft, True)
    net.cuda()
    net.eval()
    image = torch.randn(8, 1, 480, 854).cuda()
    image_size = image.shape
    scores_l3 = torch.randn(8, 1, 60, 107).cuda()
    scores_l4 = torch.randn(8, 1, 30, 54).cuda()
    feature_extractor = ResnetFeatureExtractor(name="resnet101").to("cuda:0")
    feature = feature_extractor(image)
    out = net(scores_l3, scores_l4, feature, image_size)
    print('out:', out.shape)