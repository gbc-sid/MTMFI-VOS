...
class Tracker(nn.Module):
    ...
    def __init__(self, augmenter: ImageAugmenter, feature_extractor, disc_params_l3, disc_params_l4,
        ...
        self.current_masks_l3 = None
        self.current_masks_l4 = None
        self.num_objects = 0
    ...
    def run_dataset(self, dataset, out_path, speedrun=False, restart=None):
        ...
        l3_path = self.visual_path + '/img_l3'
        l4_path = self.visual_path + '/img_l4'
        ...
        for sequence in dataset:
            ...
            outputs, seq_fps, outputs_l3, outputs_l4 \
                = self.run_sequence(sequence, speedrun)
            dset_fps.update(seq_fps)

            dst.mkdir(exist_ok=True)
            for lb, f in zip(outputs, sequence.frame_names):
                imwrite_indexed(dst / (f + ".png"), lb)
            for lb, f in zip(outputs_l3, sequence.frame_names):
                imwrite_indexed(l3_path +'/' +  f + '.png', lb)
            for lb, f in zip(outputs_l4, sequence.frame_names):
                imwrite_indexed(l4_path +'/' +  f + '.png', lb)
    ...
    def run_sequence(self, sequence, speedrun=False):
        ...
        for i, (image, labels, new_objects) in tqdm.tqdm(enumerate(sequence), desc=sequence.name, total=len(sequence),
            ...
            if len(old_objects) > 0:
                ...
                masks, masks_l3, masks_l4 = self.current_masks, \
                                            self.current_masks_l3, \
                                            self.current_masks_l4
                if len(sequence.obj_ids) == 1:
                    labels = object_ids[(masks[1:2] > 0.5).long()]
                    labels_l3 = object_ids[(masks_l3[1:2] > 0.5).long()]
                    labels_l4 = object_ids[(masks_l4[1:2] > 0.5).long()]
                else:
                    ...
                    masks_l3 = torch.clamp(masks_l3, 1e-7, 1 - 1e-7)
                    masks_l3[0:1] = torch.min((1 - masks_l3[1:]), dim=0, keepdim=True)[0]  # background activation
                    segs_l3 = F.softmax(masks_l3 / (1 - masks_l3), dim=0)  # s = one-hot encoded object activations
                    labels_l3 = object_ids[segs_l3.argmax(dim=0)]
                    masks_l4 = torch.clamp(masks_l4, 1e-7, 1 - 1e-7)
                    masks_l4[0:1] = torch.min((1 - masks_l4[1:]), dim=0, keepdim=True)[0]  # background activation
                    segs_l4 = F.softmax(masks_l4 / (1 - masks_l4), dim=0)  # s = one-hot encoded object activations
                    labels_l4 = object_ids[segs_l4.argmax(dim=0)]

            if isinstance(labels, list) and len(labels) == 0:  # No objects yet
                labels = image.new_zeros(1, *image.shape[-2:])

            outputs.append(labels)
            outputs_l3.append(labels_l3)
            outputs_l4.append(labels_l4)
        ...
        return outputs, fps, outputs_l3, outputs_l4

    def initialize(self, image, labels, new_objects):

        self.current_masks_l4 = self.current_masks_l3 = self.current_masks = torch.zeros((len(self.targets_l3) + len(new_objects) + 1, *image.shape[-2:]),
                                         device=self.device)
        ...
        for obj_id in new_objects:
            ...
            self.current_masks_l4[target_l4.index] = mask
            self.current_masks_l3[target_l4.index] = mask
    ...
    def track(self, image):
        ...
        #
                ...
                self.current_masks_l3[target_l4.index] = img_l3
                self.current_masks_l4[target_l4.index] = img_l4
        ...
        return self.current_masks, self.current_masks_l3, self.current_masks_l4
